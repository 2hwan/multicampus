...
#
#
# def modify(id,name,age,addr) :
#     conn = get_connection()
#     sql = '''update student_table set stu_name = %s, stu_age = %s, stu_addr = %s where stu_idx =%s'''
#
#     # if id != None and len(id) > 0:  # 검색어가 있을 경우
#     #     sql += '  where stu_idx =%s'
#
...
#     cursor = conn.cursor()
#
#     cursor.execute(sql,(name,age,addr,id))
#     conn.commit()
#     conn.close()
#
# def delete_student(id) :
#     conn = get_connection()
#     sql = '''delete from student_table where stu_idx = %s'''
#     sql2 = '''delete from point_table where point_stu_idx = %s'''
#
#     cursor = conn.cursor()
#
...
def add_air(df) :
    ...
    goo = ['''계양구''', '''남구''', '''남동구''', '''동구''', '''부평구''', '''서구''', '''연수구''', '''중구''']
    sql1 = '''select dateTime from finedust.'''
    sql2 = ''' order by dateTime desc limit 1'''
    sql3 = '''insert into '''
    sql4 = ''' (dateTime, pm10, pm25, so2, co, o3, no2)
                          values (%s, %s, %s, %s, %s, %s, %s)'''

    for i in range(0, 8):
        conn = get_connection()
        cursor = conn.cursor()
        cursor.execute(sql1 + goo[i] +sql2)
        result = cursor.fetchone()
        # print(str(i) +','+ df.iloc[i][6] +' ' + result[0])
        if result[0] == df.iloc[i][6] :
            conn.close()
        else :
            cursor.execute(sql3 + goo[i] + sql4, (df.iloc[i][6], df.iloc[i][0],df.iloc[i][1],df.iloc[i][2],df.iloc[i][3],df.iloc[i][4],df.iloc[i][5]))
            conn.commit()
            conn.close()
